...
type Model = Pick<Api.VibeCoding.publishVibeCodingForm, 'coverUrl'>;
...
function restoreFileLists() {
  ...
  screenshotFileList.value = form.value.screenshots.map(
    (url, i) => ({ name: `screenshot-${i + 1}.png`, url }) as UploadFile,
  );
}
